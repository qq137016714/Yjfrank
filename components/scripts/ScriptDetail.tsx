...
import { X, GitBranch, Loader2, BarChart2 } from 'lucide-react'
...
export default function ScriptDetail({ scriptId, onClose, onEdit, onIterate, canEdit }: ScriptDetailProps) {
  ...
  if (loading || !script) {
    return (
      <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
        <div className="bg-white rounded-2xl w-full max-w-xl p-12 flex items-center justify-center shadow-xl">
  ...
  const hasData = script.stat && script.stat.matchedRows > 0
    ...
    <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl w-full max-w-xl max-h-[90vh] overflow-y-auto shadow-xl">
        ...
        <div className="flex border-b px-6">
          {[
            { key: 'info', label: '脚本信息' },
            { key: 'data', label: `投放数据${hasData ? '' : ''}` },
          ].map(tab => (
            <button key={tab.key} onClick={() => setActiveTab(tab.key as 'info' | 'data')}
              ...
              {tab.label}
              {tab.key === 'data' && hasData && (
                <span className="ml-1.5 px-1.5 py-0.5 text-xs bg-blue-100 text-blue-600 rounded-full">有数据</span>
              )}
            </button>
          ))}
        </div>
                ...
                <div className="bg-gray-50 rounded-lg p-3 space-y-2">
                  ...
                  {script.children && script.children.length > 0 && (
                    <div className="flex items-start gap-2 text-sm">
                      ...
                      <span className="text-gray-500 shrink-0">衍生脚本：</span>
            ...
            <>
              {!hasData ? (
                <div className="text-center py-12 text-gray-400">
                  ...
                  <p className="text-sm">该脚本暂无匹配的投放数据</p>
                  <p className="text-xs mt-1">上传含该脚本名的 EXCEL 后自动关联</p>
                </div>
              ) : (
                <>
                  {/* 汇总指标 */}
                  ...
                  {/* 渠道明细 */}
                  {script.channelStats && script.channelStats.length > 0 && (
                    <div>
                      <p className="text-sm font-medium text-gray-700 mb-2">按渠道明细</p>
                        ...
                        <table className="w-full text-xs">
                          <thead className="bg-gray-50 text-gray-500">
                            <tr>
                              ...
                              <th className="text-right px-3 py-2">获客</th>
                              <th className="text-right px-3 py-2">ROI</th>
                            </tr>
                          </thead>
                          <tbody className="divide-y divide-gray-100">
                            {script.channelStats.map(cs => (
                              <tr key={cs.channel} className="hover:bg-gray-50">
                                <td className="px-3 py-2 font-medium text-gray-700">{cs.channel}</td>
                                <td className="px-3 py-2 text-right text-gray-600">{money(cs.totalCost)}</td>
                                <td className="px-3 py-2 text-right text-gray-600">{cs.customers.toLocaleString()}</td>
                                <td className="px-3 py-2 text-right text-gray-600">
                                  {cs.roi != null ? cs.roi.toFixed(3) : '—'}
                                </td>
                              </tr>
                            ))}
                          </tbody>
